...
function addFrameAndTag(color, tag) {
  ...
  if (document.body) {
    ...
    console.log(
      `URL Color Tagger: Added frame (${color}) and tag (${tag}) for ${window.location.href}`,
    );
  } else {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.body.appendChild(tagElement);
      console.log(
        `URL Color Tagger: Added frame (${color}) and tag (${tag}) for ${window.location.href} after DOMContentLoaded`,
      );
    });
  }
}
