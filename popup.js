...
function displayHistory() {
    ...
    chrome.storage.sync.get(null, (items) => { // Get all items
        if (chrome.runtime.lastError) {
            console.error("Error retrieving history:", chrome.runtime.lastError);
            historyList.innerHTML = '<li class="no-history">Error loading history.</li>';
            return;
        }
        ...
        urls.forEach(url => {
            ...
            if (itemData && typeof itemData.color !== 'undefined' && typeof itemData.tag !== 'undefined') { // Check if data structure is as expected
                const listItem = document.createElement('li');
                listItem.setAttribute('data-url', url); // Store URL on the li for reference

                // Truncate URL for display
                let displayUrl = url;
                if (displayUrl.length > 35) { // Adjust length as needed
                    displayUrl = url.substring(0, 35) + '...';
                }

                // Create editable fields and buttons
                listItem.innerHTML = `
                ...
                // Add event listeners for buttons within this specific list item
                listItem.querySelector('.save-changes-button').addEventListener('click', handleSaveChanges);
                listItem.querySelector('.delete-button').addEventListener('click', handleDelete);

                historyList.appendChild(listItem);
            } else {
                 console.warn(`Skipping item with unexpected data structure for URL: ${url}`, itemData);
...
function handleSaveChanges(event) {
    ...
    if (!newTag) {
        ...
        setTimeout(() => { statusField.textContent = ''; }, 2000);
        return;
    }
    ...
    button.textContent = '...';
    statusField.textContent = ''; // Clear previous status
...
function handleDelete(event) {
    ...
    if (!urlToDelete || !statusField) return;

    // Optional: Ask for confirmation
    // if (!confirm(`Are you sure you want to delete the tag for ${urlToDelete}?`)) {
    //     return;
    // }
    ...
    chrome.storage.sync.remove(urlToDelete, () => {
        if (chrome.runtime.lastError) {
            ...
            console.log('Item deleted:', urlToDelete);
            // Remove item from the list visually
            listItem.remove();

             // Check if history is now empty and show placeholder
            if (historyList.children.length === 0) {
             ...
             setTimeout(() => { if (statusDiv.textContent === 'Item deleted.') statusDiv.textContent = ''; }, 2000);
        }
    });
}
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        if (tabs && tabs.length > 0) {
            ...
            if (currentTab.url && currentTab.url.startsWith('http')) {
                ...
                const displayUrl = fullUrl.length > 45 // Adjusted length for main tab display
...
saveButton.addEventListener('click', () => {
    ...
    if (!tag) { /* ... error handling ... */ return; }

    const dataToSave = { [url]: { color: color, tag: tag } };
    ...
    chrome.storage.sync.set(dataToSave, () => {
        if (chrome.runtime.lastError) {
            ...
            // No need to refresh history here unless the user immediately switches tabs
            // displayHistory(); // Could add if desired, but might be redundant
        }
         // Clear status after delay
         setTimeout(() => { if (statusDiv.textContent === 'Saved!') statusDiv.textContent = ''; }, 2000);
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.tab-button[data-tab="main-tab"]').classList.add('active');
    document.getElementById('main-tab').classList.add('active');

    // Load data for the main tab
    loadCurrentTabData();
    // Load history data for the (initially hidden) saved tags tab
    // displayHistory(); // Load history initially so data is ready if user clicks tab
    // Let's defer loading history until the tab is clicked for potentially faster popup opening
});
